...
class GMSH_OMeshingConfig(BaseModel):
    ...
    nr_farfield: int = Field(70, gt=5, description="Number of radial cells from the airfoil surface out to the farfield boundary")
    radial_growth_ratio: float = Field(1.08, gt=1.01, le=1.3, description="The geometric growth ratio for radial cell progression from the airfoil surface to the farfield boundary")
...
class GMSH_In(BaseModel):
    ...
    def _resolve_meshing_config(cls, raw):
        ...
        if config is None:
            raw["meshing_config"] = expected()
        elif isinstance(config, dict):
            raw["meshing_config"] = expected(**config)
